...
function AvatarModel({ lipSyncRef, isSpeaking, isThinking }) {
  ...
  useEffect(() => {
    ...
    if (!cameraReady.current) {
      ...
      // Focus on upper face area: top 15% of model
      const headY = box.max.y - size.y * 0.10;

      // Calculate distance to frame the head nicely
      const fov = 30;
  ...
  useFrame((state, delta) => {
    ...
    if (!hasMorphTargets.current && jawBoneRef.current) {
      if (lipSync && isSpeaking) {
        const weights = lipSync.getVisemeWeights(0.35);
        const jawOpen =
          (weights.visemeAA || 0) * 0.8 +
          (weights.visemeO || 0) * 0.6 +
          (weights.visemeE || 0) * 0.4 +
          (weights.visemeU || 0) * 0.5 +
          (weights.visemeI || 0) * 0.3 +
          (weights.visemeOH || 0) * 0.7;
    ...
    if (!hasMorphTargets.current && !jawBoneRef.current && headBoneRef.current) {
      if (lipSync && isSpeaking) {
        const weights = lipSync.getVisemeWeights(0.35);
        const energy =
          (weights.visemeAA || 0) + (weights.visemeO || 0) +
          (weights.visemeE || 0) + (weights.visemeU || 0);
