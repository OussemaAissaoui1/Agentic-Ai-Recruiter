...
/** Parse a single CV → raw text. Used for the screening → interview handoff. */
export async function parseCv(file: File): Promise<{ id: string; text: string }> {
  const form = new FormData();
  form.append("file", file);
  const res = await fetch("/api/matching/parse", { method: "POST", body: form });
