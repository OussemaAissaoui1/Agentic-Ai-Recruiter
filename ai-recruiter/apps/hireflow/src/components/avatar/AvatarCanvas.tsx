/**
 * AvatarCanvas — full-body 3D avatar with lip-sync + idle/speaking gestures.
 * Ported from /apps/_legacy/demo/src/AvatarCanvas.jsx (line-for-line).
 */
...
import type { LipSyncAudioQueue } from "@/lib/AudioLipSync";
...
const NATURAL_POSE: Record<string, Vec3> = {
  RightShoulder: { x: 0.0, y: 0.0, z: -0.06 },
  LeftShoulder:  { x: 0.0, y: 0.0, z:  0.06 },
  RightArm:      { x: 0.0, y: 0.0, z:  0.0 },
  LeftArm:       { x: 0.0, y: 0.0, z:  0.0 },
  RightForeArm:  { x: 0.15, y: 0.0, z: 0.0 },
  LeftForeArm:   { x: 0.15, y: 0.0, z: 0.0 },
  RightHand:     { x: 0.0, y: 0.0, z: 0.0 },
  LeftHand:      { x: 0.0, y: 0.0, z: 0.0 },
};

function computeArmHangOffsets(
  boneMap: Record<string, THREE.Bone>,
  origRot: Record<string, Vec3>,
): Record<string, Vec3> {
  const results: Record<string, Vec3> = {};
  for (const armName of ["LeftArm", "RightArm"]) {
    ...
    const boneY = new THREE.Vector3(0, 1, 0)
      .applyQuaternion(new THREE.Quaternion().setFromRotationMatrix(bone.matrixWorld))
      .normalize();

    const target = new THREE.Vector3(0, -0.985, -0.174).normalize();
...
type GestureKeyframe = { t: number } & Record<string, Vec3 | number>;
type Gesture = { duration: number; bones: GestureKeyframe[] };

const GESTURE_LIBRARY: Gesture[] = [
  {
    duration: 2.2,
    bones: [
      { t: 0.0, RightForeArm: { x: 0.0,  y: 0.0,  z: 0 }, RightHand: { x: 0.0,  y: 0.0,  z: 0.0 } },
      ...
      { t: 1.8, RightForeArm: { x: -0.04, y: 0.01, z: 0 }, RightHand: { x: -0.01, y: 0.0,  z: 0.01 } },
      { t: 2.2, RightForeArm: { x: 0.0,  y: 0.0,  z: 0 }, RightHand: { x: 0.0,  y: 0.0,  z: 0.0 } },
    ],
  },
  {
    duration: 3.0,
...
function interpolateGesture(gesture: Gesture, elapsed: number): Record<string, Vec3> {
  const kf = gesture.bones;
  if (elapsed <= kf[0].t) return kf[0] as unknown as Record<string, Vec3>;
  if (elapsed >= kf[kf.length - 1].t) return kf[kf.length - 1] as unknown as Record<string, Vec3>;
  let a = kf[0]; let b = kf[1];
  ...
  for (const bone of allBones) {
    const av = (a[bone] as Vec3) || { x: 0, y: 0, z: 0 };
    const bv = (b[bone] as Vec3) || { x: 0, y: 0, z: 0 };
    result[bone] = {
      x: av.x + (bv.x - av.x) * s,
      y: av.y + (bv.y - av.y) * s,
      z: av.z + (bv.z - av.z) * s,
...
const FINGER_REST_CURL: Record<string, Vec3> = {
...
interface AvatarModelProps {
  lipSyncRef: RefObject<LipSyncAudioQueue | null>;
  isSpeaking: boolean;
  isThinking: boolean;
  controlsRef: RefObject<ControlsLike | null>;
}

function AvatarModel({ lipSyncRef, isSpeaking, isThinking, controlsRef }: AvatarModelProps) {
  const { scene } = useGLTF(AVATAR_URL);
  const { camera } = useThree();

  const meshesRef          = useRef<THREE.Mesh[]>([]);
  const bonesRef           = useRef<Record<string, THREE.Bone>>({});
  const origRotRef         = useRef<Record<string, Vec3>>({});
  ...
  useEffect(() => {
    ...
    scene.traverse((child) => {
      ...
      if (bone.isBone) boneMap[bone.name] = bone;
    });
    ...
    if (!cameraReady.current) {
      ...
      persp.position.set(center.x, targetY, center.z + Math.max(dist, 0.8));
      persp.fov = fov; persp.near = 0.01; persp.far = 60;
  ...
  const setBoneWithPose = (boneName: string, gestureOffset: Vec3 | undefined, lerpFactor: number) => {
    const bone = bonesRef.current[boneName];
    const orig = origRotRef.current[boneName];
    if (!bone || !orig) return;
  ...
  const setBoneOffset = (boneName: string, ox: number, oy: number, oz: number, lerpFactor: number) => {
    const bone = bonesRef.current[boneName];
    const orig = origRotRef.current[boneName];
    if (!bone || !orig) return;
  ...
  useFrame((state, delta) => {
    ...
    if (lipSync && isSpeaking) {
      ...
      rawAmp = vals.reduce((s, v) => s + v, 0) / Math.max(vals.length, 1);
    }
    ...
    const gestureOffsets = gestureRef.current
      ? interpolateGesture(gestureRef.current, gestureTimeRef.current)
      : ({} as Record<string, Vec3>);

    if (meshes.length > 0) {
      if (lipSync && isSpeaking) {
        const weights = lipSync.getVisemeWeights(0.4) as Record<string, number>;
        for (const mesh of meshes) {
          ...
          if (jawIdx !== undefined) {
            const jaw = (weights["viseme_aa"] || 0) * 0.18 +
                        (weights["viseme_O"] || 0) * 0.14 +
                        (weights["viseme_E"] || 0) * 0.07 +
                        (weights["viseme_U"] || 0) * 0.10;
        ...
        for (const mesh of meshes) {
          ...
          for (const key of Object.keys(dict)) {
            if (key.startsWith("viseme_") || key === "jawOpen" || key === "mouthOpen") {
              const idx = dict[key];
              if (idx !== undefined && infl[idx] > 0.002) {
                infl[idx] *= 0.78;
                if (infl[idx] < 0.002) infl[idx] = 0;
              }
            }
          }
        }
      ...
      if (blinkTimerRef.current > 0) {
        ...
        for (const mesh of meshes) {
          ...
          for (const n of ["eyeBlinkLeft", "eyeBlinkRight"]) {
            const idx = dict[n];
            if (idx !== undefined) infl[idx] = w * 0.9;
          }
        }
      }
      ...
      for (const mesh of meshes) {
        ...
        for (const n of ["mouthSmileLeft", "mouthSmileRight"]) {
          const idx = dict[n];
          if (idx !== undefined) infl[idx] = THREE.MathUtils.lerp(infl[idx], smileBase + breathe, 0.04);
        }
        for (const n of ["cheekSquintLeft", "cheekSquintRight"]) {
          const idx = dict[n];
          if (idx !== undefined) infl[idx] = THREE.MathUtils.lerp(infl[idx], cheekBase + breathe * 0.5, 0.03);
        }
        for (const n of ["eyeSquintLeft", "eyeSquintRight"]) {
          const idx = dict[n];
          if (idx !== undefined) infl[idx] = THREE.MathUtils.lerp(infl[idx], eyeSquint, 0.03);
        }
        ...
        for (const n of ["mouthDimpleLeft", "mouthDimpleRight"]) {
          const idx = dict[n];
          if (idx !== undefined) infl[idx] = THREE.MathUtils.lerp(infl[idx], 0.10, 0.03);
        }
      }
    }
    ...
    if (isSpeaking) {
      setBoneOffset("Head",
        Math.sin(t * 1.8) * 0.016 + Math.sin(t * 0.7) * 0.006,
        Math.sin(t * 1.1) * 0.022 + Math.cos(t * 0.5) * 0.007,
        Math.sin(t * 0.9) * 0.007, 0.06);
    } else {
      const ii = isThinking ? 0.011 : 0.005;
      setBoneOffset("Head",
        Math.sin(t * 0.40) * ii,
        Math.sin(t * 0.25) * ii * 0.6,
        Math.cos(t * 0.35) * ii * 0.3, 0.03);
    }
    if (isSpeaking) {
    ...
    const nat_rh = NATURAL_POSE["RightHand"];
    const nat_lh = NATURAL_POSE["LeftHand"];
    const orig_rh = origRotRef.current["RightHand"];
    const orig_lh = origRotRef.current["LeftHand"];
    ...
    for (let i = 0; i < FINGER_BONES.length; i++) {
      const name = FINGER_BONES[i];
      const bone = bonesRef.current[name];
      const orig = origRotRef.current[name];
      if (!bone || !orig) continue;
      const rest = FINGER_REST_CURL[name] || { x: 0, y: 0, z: 0 };
      const phase = i * 1.37;
      let fidgetX: number, fidgetY: number, fidgetZ: number;
      if (isSpeaking) {
...
export default function AvatarCanvas({ lipSyncRef, isSpeaking, isThinking }: AvatarCanvasProps) {
  ...
  return (
    <div style={{
      ...
      >
        ...
        {/* drei OrbitControls instance carries .target / .update — matches ControlsLike */}
        <OrbitControls
          // @ts-expect-error drei's ref type widens to OrbitControls; ControlsLike is a structural subset.
          ref={controlsRef}
          enablePan={false} enableZoom={true} enableRotate={true}
          minDistance={0.6} maxDistance={1.8}
          minPolarAngle={Math.PI / 3} maxPolarAngle={Math.PI / 2.2}
        />
      </Canvas>
      <div style={{
