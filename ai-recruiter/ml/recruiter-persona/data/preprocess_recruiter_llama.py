#!/usr/bin/env python3
"""Clean and preprocess recruiter interview CSV data for Llama fine-tuning.
...
Outputs:
  ...
  - data_v2/final/recruiter_data_report.md
...
SYSTEM_PROMPT = (
    ...
    "and keep the tone concise, respectful, and role-relevant. Avoid discriminatory or "
    "illegal questions."
)
...
def normalize_assistant_name(text: str) -> str:
    ...
    # Common interviewer/recruiter names to replace
    # This pattern matches standalone names (with word boundaries)
    # but avoids replacing names in email addresses or longer contexts
    common_names = [
    ...
    for name in common_names:
        # Match name at word boundaries, case-insensitive
        # Replace "Hi Rachel" -> "Hi Alex", "Good morning, Rachel" -> "Good morning, Alex"
        text = re.sub(
            r'\b' + name + r'\b',
            'Alex',
            text,
...
def clean_turn_text(text: str) -> str:
    # Strip occasional JSON wrappers that leak from source serialization.
    text = text.strip()
    text = JSON_ROLE_WRAPPER_RE.sub("", text)
    text = JSON_ROLE_PREFIX_RE.sub("", text)
    text = JSON_TRAILING_WRAPPER_RE.sub("", text)

    # Remove markdown emphasis markers that do not add semantic content.
    text = text.replace("**", "")
    text = text.replace("__", "")
    text = text.replace("`", "")

    # Remove bullet-like marker if left at the beginning.
    text = re.sub(r"^\s*[-*]+\s*", "", text)

    # Remove sentiment annotations and stage directions (text in parentheses)
    # e.g., "(pauses)", "(confident)", "(smiling)", "(pauses again)", etc.
    text = re.sub(r'\([^)]*\)', '', text)

    # Normalize turns that start with a doctor's name to the recruiter alias.
    text = re.sub(
        r"^\s*dr\.?\s+[A-Za-z][A-Za-z .'-]{0,80}(?=\s*,|\s*:|\s+-|\s+\(|$)",
        "Alex",
        text,
        flags=re.IGNORECASE,
    )

    # Replace various interviewer names with Alex for consistency
...
def looks_like_metadata_turn(text: str) -> bool:
    ...
    # Pure name line such as "Mrs. Tracy Smith DVM".
    if words(low) <= 7 and looks_like_person_name(low):
        return True

    if words(low) <= 14 and any(
        hint in low
        for hint in (
            "candidate",
            "interviewer",
    ...
    # Handles lines like "rachel lee, project manager" or
    # "chaaya kaur, data scientist".
    ...
    # Handles role/title-only opening lines that start with a Dr. prefix.
...
def infer_speaker_roles(turns_raw: list[tuple[str, str]], candidate_name: str) -> dict[str, str | None]:
    ...
    # If candidate is known and exactly two speakers exist after stage removal, map other as interviewer.
    ...
    # Fallback: question-heavy speakers are likely interviewers.
...
def parse_transcript(transcript: str, candidate_name: str) -> list[dict[str, str]]:
    ...
    for line in transcript.splitlines():
        ...
        # Normalize markdown-style speaker labels like **Interviewer:**
    ...
    # Trim to first interviewer turn.
    while merged and merged[0]["from"] != "gpt":
        merged.pop(0)

    # Keep last turn as interviewer for SFT style.
...
def deduplicate(examples: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], int]:
    ...
    for ex in examples:
        ...
        soft_payload = "||".join(
            f"{t['from']}:{canonicalize(t['value'])}" for t in conv[:4]
        )
...
def to_messages_record(example: dict[str, Any]) -> dict[str, Any]:
    messages = [{"role": "system", "content": example["system"]}]
    for t in example["conversations"]:
        role = "assistant" if t["from"] == "gpt" else "user"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Preprocess recruiter CSV for Llama SFT")
    ...
    train_messages = [to_messages_record(x) for x in train]
    eval_messages = [to_messages_record(x) for x in evals]
    train_sharegpt = [to_sharegpt_record(x) for x in train]
    eval_sharegpt = [to_sharegpt_record(x) for x in evals]

    train_messages_path = out_dir / "recruiter_llama_train_messages.jsonl"
    eval_messages_path = out_dir / "recruiter_llama_eval_messages.jsonl"
    train_sharegpt_path = out_dir / "recruiter_llama_train_sharegpt.jsonl"
    eval_sharegpt_path = out_dir / "recruiter_llama_eval_sharegpt.jsonl"

    write_jsonl(train_messages_path, train_messages)
    write_jsonl(eval_messages_path, eval_messages)
    write_jsonl(train_sharegpt_path, train_sharegpt)
    write_jsonl(eval_sharegpt_path, eval_sharegpt)
    ...
    gpt_questions = [
        sum(1 for t in x["conversations"] if t["from"] == "gpt" and "?" in t["value"])
        for x in deduped
    ]
    ...
    report_lines.extend(
        [
            ...
            "",
            "## Output Files",
            f"- `{train_messages_path}`",
            f"- `{eval_messages_path}`",
            f"- `{train_sharegpt_path}`",
            f"- `{eval_sharegpt_path}`",
            f"- `{stats_path}`",
        ]
    )
